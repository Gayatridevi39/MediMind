...
def generate_answer(question, report_text):
    ...
    return qa_chain.run(report_text=report_text, question=question)

if st.button("Generate Answer", type="primary") and qa:
    if "extracted_text" in st.session_state:
        with st.spinner("🧠 Analyzing and answering..."):
            try:
                answer = generate_answer(qa, st.session_state["extracted_text"])
                st.subheader("🔎 Answer:")
                st.markdown(answer)
...
def generate_summary(report_text, language):
    ...
    summary = summary_chain.run(report_text=report_text)
